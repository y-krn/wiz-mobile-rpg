...
**Logged**: 2026-07-22T13:12:46Z
**Priority**: high
**Status**: resolved
**Area**: frontend

### Summary
`aspect-ratio: auto`だけでは、基底ルールで失われたflex growは復元されない。

### Details
`#viewport-panel`を`flex: 0 1 auto`へ変更後、result/eventモードで
`aspect-ratio: auto`を指定してもパネルは利用可能高まで伸びず、canvasのintrinsic
400x260由来の高さへ縮んだ。両モードの絶対配置overlayも同じ包含ブロック内で潰れた。

### Suggested Action
共有flex itemの基底`flex`を変更する際、特別モードで`aspect-ratio`、`max-height`、
`min-height`だけでなく`flex-grow`も復元する。テストは宣言CSSではなく、通常モードと
特別モード双方のcomputed heightを利用可能高から算出して検証する。

### Metadata
- Source: user_feedback
- Related Files: src/styles/app-shell.css, tests/verify-viewport-stability.spec.js
- Tags: CSS, flexbox, aspect-ratio, viewport, layout-regression
- Pattern-Key: harden.flex_special_mode_override
- Recurrence-Count: 1
- First-Seen: 2026-07-22
- Last-Seen: 2026-07-22

### Resolution
- **Resolved**: 2026-07-22T13:12:46Z
