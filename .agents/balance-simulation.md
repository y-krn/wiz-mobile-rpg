...
- 再測定対象: #461基準線、#470/#471/#475の完成ビルド・core監視、#468/#473の罠/解除監査、#480の罠方針比較。
  #264の傷薬本数掃引・回復単価掃引は採用値で再測定し、到達性の符号・順位を確認した。詳細は
...
Q4−Q1 B5死亡率差は **−6.7pt [−10.1, −3.2]**、Q4安全性と職内centered条件は成立したが、
隣接差CIを含む単調減少条件は不成立で、runnerのA1判定は **不成立**。これは戦闘方針変更後の
再測定結果であり、#470/#475の完成ビルド受入判断を自動で維持・反転しない。
...
- 次回対象: #470完成ビルド定義、#471 core装備率監視、#475 A1/A3、#468/#473 罠/解除監査、#480 罠方針比較。
  それぞれ同じ採用envで再測定し、旧#485値と混ぜずに比較する。
