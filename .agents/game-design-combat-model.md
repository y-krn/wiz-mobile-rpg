...
| ---: | --- | --- | --- | --- |
| 1 | 物理は旧来 `-floor(def/2)`、呪文は `×(1-magicResist)` | **結論（#732、#966で再校正）** | 旧式は `def` と `magicResist` が別 input・別順序で、同じ低 damage 帯に別の clamp を持っていた。現行は `defResistance = def/(def+k_direction)` とし、`physResist` と -1〜0.9 の加算poolへ統合する。公式値はプレイヤー→敵 `k_out=40`、敵→プレイヤー `k_in=8`。#966のformula-scope比較では scale=8 が B1/B5 の典型Mageを約11〜14%、B8/B11の高ATK通常敵を約8〜10%に保ち、scale=2のDEF14での1ダメージ率100%を解消した。scale=10/12は序盤高ATK帯を20%超へ押し上げるため採用しない。#716の段階表示は active DEF buff/debuff を含む `getEffectiveDef` から求めた最終physical poolを読む。 | 決定 1 |
...
再校正後の公式値は、プレイヤー→敵を `k_out=40`、敵→プレイヤーを `k_in=8` とする。
