...
node scratch/simulations/sim_issue_404_affix_volume.js
```
...
node scratch/simulations/sim_issue_404_workshop_buyout.js
...
`scratch/simulations/sim_depth_material_ev.js`のslot modeとempty-slot snapshot filterを変更したため、
厳密なrunner/source SHA比較では次の既存測定が再取得対象になる。今回はIssue #409のbefore/after
と標準モード対照だけを取り直した。

- `evidence/results/issue-404-affix-volume.md` の base / conservative / balanced / high / upper
- `evidence/results/issue-446-slot-vs-affix.md`
