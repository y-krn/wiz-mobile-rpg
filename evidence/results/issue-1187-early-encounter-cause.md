...
- Source: `1f82ee4d98fc1812551727803fe91926a315dcab`
...
- Runner: `issue1187-early-encounter-cause-v1`, schema 1
...
- Provenance: `originMainAncestor: true`, `workingTreeClean: true`, runner diff SHA-256 `c32a7a99d8b771215258b8d1f989951a568771b63eb1b8466f510abb8f2983bd`
...
pairs fall from 94.73% at full HP to 0.23% at 25% HP. In the natural run,
ordinal-2 death was 248/493 (50.30%) versus 277/922 (30.04%) at ordinal 1,
consistent with first-fight Cost carrying into the next encounter. This does
not establish that a free heal is the right fix; it identifies resource/recovery
as a separate follow-up axis.
...
- **A — strongly supported, with composition as the first boundary.** Pair
  exposure is 26.2% in encounter 1 and 14.7% in encounter 2, with 61.45% and
  87.07% pair lethality versus 17.58% and 34.68% for singles. Suppressing
  early pairs in matched counterfactuals moves B2 arrival by 1.9–3.3 points
  while keeping the rest of the production path intact.
- **B — supported as a contributing carryover axis, not isolated as the only
  cause.** Fixed HP bands and the ordinal-2 natural-run collapse show entry
  resource pressure; recovery was intentionally not modeled.
...
The next production change should be a separate small Issue for **early
encounter composition cadence / pair scheduling**, with a fresh matched
measurement and manual fresh-save playtest. This Issue does not authorize or
recommend a global enemy nerf, blanket two-enemy removal, free recovery, or a
B1F-only combat rule. Keep #1184 open until that production candidate is
implemented, remeasured, and manually playtested.
...
- Targeted #1186, #1151, and #1187 regressions passed; unit fast passed 183
  tests with 6 intentional skips before the final helper-only amend.
