...
`tests/ui-golden-journeys.spec.js` and compare only stable surfaces. Playwright's
default platform-specific snapshot naming is intentional:

- Canonical CI baseline (Ubuntu + repository Chromium):
  - `tests/ui-golden-journeys.spec.js-snapshots/golden-town-390-linux.png`
  - `tests/ui-golden-journeys.spec.js-snapshots/golden-preparation-390-linux.png`
  - `tests/ui-golden-journeys.spec.js-snapshots/golden-result-390-linux.png`
- macOS companion baseline:
...
The Linux images were generated by the Ubuntu browser job with the
repository-default Chromium revision. No `snapshotPathTemplate` was added, so
font-rendering differences do not force a cross-platform PNG match.
...
The focused entrypoint passes with six tests on the local Darwin runner, while
the Linux snapshots above are selected automatically by Ubuntu CI:
