...
- 同一runner `scratch/simulations/sim_issue_538_upper_spells.js`、seed=461、各職N=500、calibration N=100、current case（trapGuard70 / killHeal10 / mpWard10）で実行。`ISSUE538_SPELL_POLICY`だけ `legacy`（main時点のKATINO→HALITO固定）/`current`（上位呪文選択）へ切替。
...
```sh
SIM_RUNS=500 SIM_CALIBRATION_RUNS=100 ISSUE538_SPELL_POLICY=legacy ISSUE538_RESULT_BASENAME=issue-538-upper-spells-legacy ISSUE538_CASE_FILTER=current node scratch/simulations/sim_issue_538_upper_spells.js
SIM_RUNS=500 SIM_CALIBRATION_RUNS=100 ISSUE538_SPELL_POLICY=current ISSUE538_RESULT_BASENAME=issue-538-upper-spells-current ISSUE538_CASE_FILTER=current node scratch/simulations/sim_issue_538_upper_spells.js
```
...
```sh
node --check scratch/simulations/sim_issue_538_upper_spells.js
ISSUE538_SMOKE=1 node scratch/simulations/sim_issue_538_upper_spells.js
SIM_RUNS=500 SIM_CALIBRATION_RUNS=100 ISSUE538_SPELL_POLICY=current ISSUE538_CASE_FILTER=current,trapguard-60,trapguard-55,trapguard-50,killheal-8,killheal-6,killheal-4,mpward-8,mpward-6,mpward-4,combined-60-8-8,combined-55-8-8,combined-55-6-8,combined-50-6-6,combined-40-4-4 node scratch/simulations/sim_issue_538_upper_spells.js
```
...
- 詳細な全endpoint・A1・固定環境・ハッシュは `evidence/results/issue-461-baseline.md`を参照。
