...
測定系は #467 系。`scratch/simulations/sim_issue_271_quality_remeasure.js` から現行
`scratch/simulations/sim_depth_material_ev.js` を呼び、実 `generateRunFloor` 経路、戦闘、罠、装備、状態回復、
...
```sh
SIM_RUNS=13000 ISSUE271_RESULT_SUFFIX=issue475-final node scratch/simulations/sim_issue_271_quality_remeasure.js
```
...
- `node --check scratch/simulations/sim_issue_271_quality_remeasure.js`: PASS
- import/export存在確認、`generateRunFloor` 配線確認: PASS
- N=1・complete 1セル smoke: PASS
- `npm run lint`: PASS
- `node scratch/tests/regression/test_sim_reward_paths.js`: PASS（40 sim files）
