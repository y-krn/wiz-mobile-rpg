...
`scratch/simulations/sim_depth_material_ev.js` に、B5F の非特殊セル歩行を対象とする火炎の罠を追加した。`src/` は変更していない。既存の床罠スケジュール、`trapGuard`、`TRAP_POLICY`、罠回避・軽減経路とは分離している。
...
```sh
rg -l -i "B5 endpoint|B5代理|B5 entrant|target.?depth.?5|B5F" evidence/results/*.md | sort
```
...
- `node scratch/tests/regression/test_sim_reward_paths.js`: 成功（60 sim files）。
- `git diff --check`: 成功。変更ファイルは `scratch/simulations/sim_depth_material_ev.js` と本 summary のみ、`src/` 差分なし。
