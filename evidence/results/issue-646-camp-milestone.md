...
- `src/data/biomes.js` の4バイオーム（`eventSkins.camp` が null だった
  ...
  追加した（採用呼称と理由は後述）。
- `scratch/simulations/sim_depth_material_ev.js` に `SIM_ISSUE646_CAMP_LEVEL=1|2|3` を追加し、
...
```sh
SIM_ISSUE646_CAMP_LEVEL=1 SIM_SEED=231 node scratch/simulations/sim_depth_material_ev.js  # 水準1（採用仕様、実src）
SIM_ISSUE646_CAMP_LEVEL=2 SIM_SEED=231 node scratch/simulations/sim_depth_material_ev.js  # 水準2（測定専用）
SIM_ISSUE646_CAMP_LEVEL=3 SIM_SEED=231 node scratch/simulations/sim_depth_material_ev.js  # 水準3（測定専用）
```
...
- `npm run test:unit`: PASS（94/94。1回目の実行で `test_chest_relief.js` が単発で
  FAILしたが、直接再実行 (`node scratch/tests/unit/test_chest_relief.js`) では全件PASS、
...
- `node --check src/run_map_generator.js` / `src/data/biomes.js` /
  `scratch/simulations/sim_depth_material_ev.js`: PASS
- `floorHasCampEvent(floor)` を深度1-25で確認: true になるのは 6,11,16,21 のみ
- 深度sim掃引3水準: 3回とも exit 0 で完走（上記SHA256参照）
- `scratch/tests/unit/test_camp_waypoints.js`: 新ロジック（`isMilestoneFloor(floor-1)`）に
  合わせて期待値を更新しPASS
- `scratch/tests/regression/test_run_map_determinism.js`: 床11・21のマップハッシュがcamp配置変更
  により変化するため期待値を更新しPASS（床1は守護者関連イベントの影響を受けないため
  不変を確認）
- `scratch/tests/unit/test_heal_priority_policy.js`: `targetDepth: 5` のまま無変更で継続PASS
  （新ロジックでも床5はcamp床でないため、#641での調整の前提は崩れない）

## 実装の委譲

`src/run_map_generator.js`、`src/data/biomes.js`、`scratch/simulations/sim_depth_material_ev.js`、
`.agents/game-design.md`、および関連する2件の `scratch/tests/*/test_*.js` の期待値更新は
