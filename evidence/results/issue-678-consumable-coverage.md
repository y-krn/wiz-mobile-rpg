...
同じseed・条件で `scratch/simulations/sim_depth_material_ev.js` を2回実行したstdoutのSHA-256は、
次の値で2回とも一致した。

`70af47bbadc40567b6288b227e7d62b10d608a88dc6312a58fc61d1cfa6e8b43`

固定環境の `scratch/simulations/sim_commit_depth_624.js` は実行時間フィールドを含むためstdout全体の
...
- `SIM_RUNS=1 SIM_CALIBRATION_RUNS=1 ... node scratch/simulations/sim_depth_material_ev.js`: PASS
- `node scratch/tests/regression/test_sim_reward_paths.js`: PASS（65 files）
- `npm run lint`: PASS
...
（`scratch/simulations/sim_depth_material_ev.js:6282`）と combat item の状態治療直後
...
- command: `env -u SIM_PARALLEL SIM_RUNS=500 SIM_CALIBRATION_RUNS=100 SIM_SEED=231 node scratch/simulations/sim_depth_material_ev.js`
...
- #624-compatible current-main harness used the same full command recorded in the #689
  ...
  `ISSUE689_DETERMINISTIC=1 node scratch/simulations/sim_commit_depth_624.js`); 2,000 rows and focused audit
