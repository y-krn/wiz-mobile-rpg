...
受入基準線は通常simのデフォルト集約値ではなく、#666が記録した #624互換の固定環境で定義されている。そのため、`scratch/simulations/sim_commit_depth_624.js` が現行 `scratch/simulations/sim_depth_material_ev.js` の `simulateRun` を呼ぶ互換測定も実行した。互換測定は B21、#612の6工房分布、出発クラフト、run単位乱数分離を使用した。
