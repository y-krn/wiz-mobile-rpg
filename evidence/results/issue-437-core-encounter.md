...
- `SIM_PARALLEL` は指定していない。`scratch/simulations/sim_parallel.js` の既定値を使用した。
...
`scratch/simulations/sim_depth_material_ev.js`、`scratch/simulations/sim_workshop_progression.js`、および装備生成経路を変更したため、旧stdout SHAを監査値として再利用しない対象は次の通り。
...
- `evidence/results/issue-292-corrected-results.md` / `issue-292-sim-parallel-progress.md`
- `evidence/results/issue-270-real-src-measurement.md`
- `evidence/results/issue-271-atk-def-affix-unread.md` / `issue-271-resistance-integrity-progress.md`
...
node scratch/simulations/sim_issue_437_core_encounter.js
```
...
買い切り率の補助測定は `PROGRESSION_ONLY_REFERENCE=1 PROGRESSION_TRIALS=10 PROGRESSION_RUNS=50 PROGRESSION_CALIBRATION_RUNS=100 PROGRESSION_SEED=231 PROGRESSION_IDENTIFICATION_POLICY=powder` を追加して `node scratch/simulations/sim_workshop_progression.js`。これは買い切りrun開始点をN=500に揃えるためで、固定深度simの`SIM_RUNS=500`を代替しない。
