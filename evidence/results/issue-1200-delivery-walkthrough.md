...
| Material ambiguity found before implementation | Issue #1196 distinguishes encounter 2/3 and treats B2 arrival as secondary, but does not itself define whether runner `targetDepth: 2` means encounter ordinal 2 or B2F. The base repository code shows separate concepts: continuation rows use `encounterOrdinal` for 2/3 (`scratch/measurements/starting_kit_diagnostic.js:452-476`), the runner passes `targetDepth: 2` (`:1304-1309`), and output reports B2F arrival separately from the encounter survival curve (`:1430-1444`). |
| Readiness decision and evidence plan | This was a material semantics risk, not an already-settled Issue decision. Inspect the runner and regression boundaries before implementation; keep encounter-ordinal 2/3 recovery evidence separate from the B2F floor metric, with same-step target-encounter events excluded by `tests/node/regression/test_starting_kit_diagnostic.js:173-191`. |
