...
wall-clock は calibration と run phase を分離。CPU は run phase の user+system。raw/summary JSON は再現用 untracked output。再現コマンドは `SIM_ISSUE446_CONDITION=<condition> node scratch/simulations/sim_issue_446_slot_vs_affix.js`。
...
今回 `scratch/simulations/sim_depth_material_ev.js` に `affixless-duplicates` 条件と slot処理を追加した。既定 `standard` / `unlimited` の raw outcome は維持したが、共通runnerのソースSHAが変わったため、同一測定としてSHAを再利用できない。厳密比較が必要な過去結果:

- `evidence/results/issue-444-build-matching.md`（#445）
...
- smoke: `SIM_RUNS=10 SIM_CALIBRATION_RUNS=2 SIM_ISSUE446_CONDITION=base node scratch/simulations/sim_issue_446_slot_vs_affix.js` pass
- 4条件×7シナリオ、raw row key（scenario/run/class）重複なし pass
- `src/` 無変更
- `node scratch/tests/regression/test_sim_reward_paths.js` pass（31 files）
