...
深層7シナリオの再現コマンドは `SIM_ISSUE404_PROFILE=<profile> node scratch/simulations/sim_issue_404_affix_volume.js`（`SIM_PARALLEL` は指定しない）。
...
追加 #409 +1 の再現コマンドは `SIM_ISSUE446_CONDITION=slots-affix-plus1 SIM_AFFIXLESS_DUPLICATE_COUNT=1 SIM_AFFIXLESS_DUPLICATE_SLOT=accessory SIM_RUNS=2200 SIM_CALIBRATION_RUNS=100 node scratch/simulations/sim_issue_446_slot_vs_affix.js`（`SIM_PARALLEL` は指定しない）。
...
工房買切測定は `SIM_ISSUE404_PROFILE=<profile> PROGRESSION_TRIALS=30 PROGRESSION_RUNS=40 node scratch/simulations/sim_issue_404_workshop_buyout.js`。stdout SHA / wall / CPU は以下。
...
今回 `scratch/simulations/sim_depth_material_ev.js` に duplicate count / accessory slot filter を追加したため、既定値（count=2、slot filterなし）のraw outcomeは維持するが、厳密なrunner SHA比較では再取得対象になる。今回の追加測定に必要な過去結果は次のとおり。

- `evidence/results/issue-404-affix-volume.md` の base / conservative / balanced / high / upper の5 profile測定
- `evidence/results/issue-446-slot-vs-affix.md`（PR #447の base / unlimited / (1) / (2)）
- `evidence/results/issue-444-build-matching.md`
- `evidence/results/issue-271-quality-remeasure.md`
...
- `node scratch/tests/regression/test_sim_reward_paths.js` pass (33 sim files)
