...
test('Simple Enemy Value Review v2 captures readable four-prototype gameplay pixels @e2e @visual @smoke', async ({ page }, testInfo) => {
  for (const subject of SUBJECTS) {
    await openSimpleRich(page);
    await setCombat(page, [subject]);
    await capture(page, testInfo, `${subject.id}-simple-rich-v2.png`);
  }

  await openSimpleRich(page);
  await setCombat(page, SUBJECTS);
  await capture(page, testInfo, 'simple-enemy-gate-combined-v2.png');
  await capture(page, testInfo, 'simple-enemy-value-diagnostic-v2.png');
  ...
  await testInfo.attach('simple-enemy-value-diagnostic-v2.json', {
