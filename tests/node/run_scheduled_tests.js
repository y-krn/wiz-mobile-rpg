...
if (SCHEDULED_MANIFEST.length !== HEAVY_TEST_OWNERSHIP_COUNTS.SCHEDULED) {
  console.error(
    `[scheduled] SCHEDULED ownership count mismatch: ${SCHEDULED_MANIFEST.length}`,
  );
  process.exit(1);
}
...
console.log(`[scheduled] SCHEDULED manifest count: ${SCHEDULED_MANIFEST.length}`);
console.log(`[scheduled] execution count: ${tasks.length}`);
console.log('[scheduled] execution paths:');
...
console.log(`[scheduled] PASS ${passed.length} / FAIL ${failed.length}`);
if (failed.length > 0) {
  console.error(`[scheduled] FAIL paths: ${failedPaths.join(', ')}`);
  process.exitCode = 1;
}

if (process.env.GITHUB_STEP_SUMMARY) {
  const summary = [
    '## unit-heavy-scheduled',
    '',
    `- count: ${tasks.length}`,
