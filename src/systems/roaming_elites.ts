...
type EliteMonsterLike = {
  ...
  kind?: string;
  floor?: number;
  physResist?: number | null;
  magicResist?: number | null;
  perception?: ElitePerception;
  combatTrait?: EliteCombatTrait;
  combatTraitLabel?: string;
  trialThemeIds?: string[];
  trialRole?: string;
  spawnReason?: string;
  homeX?: number;
  homeY?: number;
};
type EliteFloorState = {
...
type EliteStoredTrial = { bandIndex: number; mainId: string; subId: string };
type EliteTrial = {
  bandIndex: number;
  main: { id: string };
  sub: { id: string };
...
type EliteFloorRole = { id: string; mainWeight: number; subWeight: number };
type EliteCurrentRunLike = {
  runSeed?: string;
  eliteFloors?: Record<string, Partial<EliteFloorState> | undefined> | null;
  eliteDefeatedFloors?: number[];
  trialBands?: Record<number, EliteStoredTrial | undefined>;
};
type EliteStateLike = {
  floor: number;
  currentRun?: EliteCurrentRunLike | null;
  roamingMonsters?: EliteMonsterLike[] | null;
  map?: EliteGrid | null;
  maps?: (EliteGrid | null | undefined)[];
  x?: number;
  y?: number;
};
...
export const ELITE_COMBAT_TRAITS: readonly EliteCombatTrait[] = Object.freeze([
  "berserk",
  "armored",
  "spell_eater",
  "regenerator",
...
export const ELITE_COMBAT_TRAIT_LABELS: Readonly<Record<EliteCombatTrait, string>> = Object.freeze({
...
const ELITE_TRAIT_THEME_WEIGHTS: Readonly<Record<string, Readonly<Partial<Record<EliteCombatTrait, number>>>>> = Object.freeze({
...
export function getEliteId(floor: number): string {
  return `RUN_ELITE_B${floor}`;
}

export function getElitePerception(runSeed: string, floor: number): ElitePerception {
  const rng = createRng(`${runSeed}:elite-perception:B${floor}`);
  return ELITE_PERCEPTIONS[Math.floor(rng() * ELITE_PERCEPTIONS.length)] as ElitePerception;
}

export function getEliteCombatTraitWeights(
  trial: EliteTrial | null,
  role: EliteFloorRole | null = null
): Record<EliteCombatTrait, number> {
  const weights = Object.fromEntries(ELITE_COMBAT_TRAITS.map(trait => [trait, 1])) as Record<EliteCombatTrait, number>;
  if (!trial) return weights;
  const floorRole = role || getFloorRole(trial.bandIndex * 5 + 1) as EliteFloorRole;
  for (const [theme, roleWeight] of ([
    ...
    for (const [trait, affinityWeight] of Object.entries(traitWeights) as [EliteCombatTrait, number][]) {
...
export function getEliteCombatTrait(runSeed: string, floor: number, storedTrial: EliteStoredTrial | null = null): EliteCombatTrait {
  const rng = createRng(`${runSeed}:elite-combat-trait:B${floor}`);
  const trial = getBandTrialForFloor(runSeed, floor, storedTrial as null) as EliteTrial;
  const weights = getEliteCombatTraitWeights(trial, getFloorRole(floor) as EliteFloorRole);
  ...
  return ELITE_COMBAT_TRAITS.at(-1) as EliteCombatTrait;
}

export function applyEliteCombatTraitStats<T extends EliteMonsterLike>(monster: T | null | undefined, combatTrait: string): T | null | undefined | (T & { physResist: number; magicResist: number }) {
...
function getEliteFloorState(currentRun: EliteCurrentRunLike, floor: number): EliteFloorState {
  ...
  return currentRun.eliteFloors[key] as EliteFloorState;
}

export function markEliteEntryRollResolved(stateLike: EliteStateLike & { currentRun: EliteCurrentRunLike }, floor: number): EliteFloorState {
...
function collectReachableKeys(grid: EliteGrid, start: ElitePosition): Set<string> {
...
export function createFloorElite({ runSeed, floor, mapData, spawnReason = "entry", spawnOrigin = null, storedTrial = null }: {
  runSeed: string;
  floor: number;
  mapData?: { grid?: EliteGrid | null } | null;
  spawnReason?: "entry" | "prolonged" | string;
...
function getFloorElite(stateLike: EliteStateLike, floor: number): EliteMonsterLike | undefined {
  return (stateLike.roamingMonsters || []).find(monster => monster.kind === "elite" && monster.floor === floor);
}

export function progressEliteThreat(stateLike: EliteStateLike): { omens: string[]; spawned: EliteMonsterLike | null } {
  const floor = stateLike?.floor;
  const currentRun = stateLike?.currentRun;
  ...
  while (floorState.prolongedChecks < checksDue) {
    ...
    const spawned = createFloorElite({
      ...
      spawnOrigin: Number.isInteger(stateLike.x) && Number.isInteger(stateLike.y)
        ? { x: stateLike.x as number, y: stateLike.y as number }
        : null
    });
    if (!spawned) continue;
