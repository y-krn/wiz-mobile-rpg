type SpellCombatSummary = {
  tag?: string;
  effect?: string;
  category?: string;
};

const SPELL_COMBAT_SUMMARIES: Readonly<Record<string, SpellCombatSummary>> = Object.freeze({
...
export function getSpellCombatSummary(spellName: unknown): SpellCombatSummary {
  return { ...(SPELL_COMBAT_SUMMARIES[spellName as string] || { tag: "不明", effect: "", category: "unknown" }) };
}
