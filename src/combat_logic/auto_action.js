...
function usesSpellPolicy(character, spellType) {
  return hasSpellType(character, spellType);
}
...
export function getPreferredOffensiveSpellName(
  ...
  if (usesSpellPolicy(character, "mage")) {
    return getMageOffensiveSpellName(monsters, canCast);
  }
  if (usesSpellPolicy(character, "priest") && canCast("BADIOS")) return "BADIOS";
  return null;
}
...
export function getAutoHealTargetIdx(character, healThreshold = AUTO_HEAL_HP_THRESHOLD) {
  if (!getActiveSpellKeys(character).some(spellName => PRIEST_HEALING_SPELLS.includes(spellName))) return null;
...
export function chooseAutoCombatAction({
  ...
  if (healingTargetIdx !== null && usesSpellPolicy(character, "priest")) {
  ...
  if (usesSpellPolicy(character, "priest") && canCast("BADIOS")) {
