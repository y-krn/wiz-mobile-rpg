...
type EquipmentHandItem = {
  type: string;
  hands?: unknown;
  name?: string;
};

type EquipmentHandsCharacter = {
  equipment?: object | string | number | boolean | null;
} | null | undefined;

type EquipmentHandsOptions = {
  replacingSlot?: unknown;
  nextItem?: unknown;
};

export const MAX_EQUIPMENT_HANDS = 2;

export function getEquipmentHands(itemOrKey: unknown): number {
  const item: EquipmentHandItem | null = getItemData(itemOrKey);
  if (!item) return 0;
...
export function getCharacterEquipmentHands(
  char: EquipmentHandsCharacter,
  { replacingSlot = null, nextItem = null }: EquipmentHandsOptions = {}
): number {
...
export function getEquipmentHandSummary(itemOrKey: unknown): string {
  const item: EquipmentHandItem | null = getItemData(itemOrKey);
  if (!item || !["weapon", "shield"].includes(item.type)) return "";
  return getEquipmentHands(item) === 2 ? "両手" : "片手";
}

export function getEquipmentHandConflict(
  char: EquipmentHandsCharacter,
  itemOrKey: unknown,
  replacingSlot: unknown = null
): { hands: number; maxHands: 2; message: string } | null {
  const item: EquipmentHandItem | null = getItemData(itemOrKey);
