...
assert.deepEqual(active.map(row => row.defResistance), [1 / 2, 1 / 2, 1 / 2, 1 / 2]);
assert.deepEqual(active.map(row => row.formulaRaw), [4, 5, 6, 7]);
assert.deepEqual(active.map(row => row.formulaDmg), [2, 2, 3, 3]);
assert.deepEqual(active.map(row => row.finalDmg), [2, 2, 3, 3]);
...
assert.equal(activeFlee.defResistance, 1 / 2);
assert.equal(activeFlee.formulaRaw, 7);
assert.equal(activeFlee.formulaDmg, 3);
assert.equal(activeFlee.finalDmg, 3);
