...
assert.deepEqual(active.map(row => row.defResistance), [2 / 3, 2 / 3, 2 / 3, 2 / 3]);
assert.deepEqual(active.map(row => row.formulaRaw), [4, 5, 6, 7]);
assert.deepEqual(active.map(row => row.formulaDmg), [1, 1, 2, 2]);
assert.deepEqual(active.map(row => row.finalDmg), [1, 1, 2, 2]);
assert.deepEqual(empty.map(row => row.finalDef), [3, 3, 3, 3]);
assert.deepEqual(empty.map(row => row.defResistance), [3 / 5, 3 / 5, 3 / 5, 3 / 5]);
assert.deepEqual(empty.map(row => row.formulaDmg), [1, 2, 2, 2]);
assert.deepEqual(empty.map(row => row.finalDmg), [1, 2, 2, 2]);
...
assert.equal(activeFlee.defResistance, 2 / 3);
assert.equal(activeFlee.formulaRaw, 7);
assert.equal(activeFlee.formulaDmg, 2);
assert.equal(activeFlee.finalDmg, 2);
assert.equal(emptyFlee.finalDef, 3);
assert.equal(emptyFlee.defResistance, 3 / 5);
assert.equal(emptyFlee.formulaDmg, 2);
assert.equal(emptyFlee.finalDmg, 2);
