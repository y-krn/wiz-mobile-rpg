...
(async () => {
  ...
  await (async () => {
    ...
    assert.strictEqual(state.party.length, 1, "Party wipe should retain exactly one runtime character");
    assert.strictEqual(state.party[0].status, "dead", "Solo character should retain dead status");
    assert.strictEqual(state.party[0].hp, 0, "Solo character should retain zero HP");
