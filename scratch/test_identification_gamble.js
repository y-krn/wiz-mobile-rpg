...
function sampleHeavyCurses({ heavyCurseShare, seed, samples = 20000 }) {
  const rng = lcg(seed);
  let heavy = 0;
  for (let i = 0; i < samples; i++) {
    const curseEffectId = pickCurseEffectId(rng, heavyCurseShare);
    heavy += CURSE_EFFECTS[curseEffectId].heavy ? 1 : 0;
  }
...
test("重呪選出率は1階0から増加し、13階以降0.6を上限とする", () => {
  assert.equal(getIdentificationGambleProfile(1).heavyCurseShare, 0);
  assert.equal(getIdentificationGambleProfile(5).heavyCurseShare, 0.2);
  assert.equal(getIdentificationGambleProfile(11).heavyCurseShare, 0.5);
  assert.equal(getIdentificationGambleProfile(13).heavyCurseShare, 0.6);
  assert.equal(getIdentificationGambleProfile(30).heavyCurseShare, 0.6);
});

test("重呪プールは純デメリット4種だけで構成される", () => {
  const heavyCurses = Object.values(CURSE_EFFECTS).filter(curse => curse.heavy);
  assert.equal(heavyCurses.length, 4);
  for (const curse of heavyCurses) {
    assert.ok(Object.values(curse.mod).every(value => value < 0), curse.id);
  }
});

test("重呪選出率が指定shareの期待レンジに収まる", () => {
  const zeroRate = sampleHeavyCurses({ heavyCurseShare: 0, seed: 18100 });
  const halfRate = sampleHeavyCurses({ heavyCurseShare: 0.5, seed: 18150 });
  assert.equal(zeroRate, 0);
  assert.ok(Math.abs(halfRate - 0.5) <= 0.05, `heavy curse=${halfRate}`);
});

test("呪い選出のrng消費回数はプールと呪い種に関係なく2回", () => {
  const cases = [
    { heavyCurseShare: 0, poolRoll: 0 },
    { heavyCurseShare: 0.5, poolRoll: 0.25 },
    { heavyCurseShare: 0.5, poolRoll: 0.75 },
    { heavyCurseShare: 1, poolRoll: 0.99 }
  ];
  for (const { heavyCurseShare, poolRoll } of cases) {
    let calls = 0;
    pickCurseEffectId(() => {
      calls++;
      return calls === 1 ? poolRoll : 0.99;
    }, heavyCurseShare);
