...
| simulation route | `scratch/sim_depth_material_ev.js` schedules and records all three cells, retries a fled boss, applies the stairs gate, and emits route, policy, spend, cure, and uncurse counters | evidenced by deterministic probe and N=500 |
...
Command used by `scratch/test_issue_895_milestone_route.js`:

```sh
node scratch/test_issue_895_milestone_route.js
```
