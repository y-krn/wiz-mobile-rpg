...
`scratch/sim_depth_material_ev.js` の `CAMP_FLOORS = new Set([2, 4])` は床番号の取り違えだった。
...
- `scratch/sim_depth_material_ev.js` は `CAMP_FLOORS` を撤去し `floorHasCampEvent` を import して使用。
...
- `scratch/test_heal_priority_policy.js` は camp 是正で `targetDepth: 21` の範囲に床6–10の camp が
...
- 修正後（`node scratch/sim_depth_material_ev.js`、コミット `80f833b`）:
...
`git checkout <commit>` で切替、それぞれ `node scratch/sim_depth_material_ev.js`
をデフォルト env で実行）:
...
- `SIM_RUNS=1 SIM_CALIBRATION_RUNS=1 node scratch/sim_depth_material_ev.js`: PASS（exit 0）
