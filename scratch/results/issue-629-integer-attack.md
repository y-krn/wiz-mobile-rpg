...
- `node --check scratch/sim_depth_material_ev.js`: OK
...
node scratch/sim_depth_material_ev.js > after.log

# before（一時的に武器項のfloorだけ外す: return ((weaponAtk + buffAtk) * 1.5 + ...) * meleeMod;）
node scratch/sim_depth_material_ev.js > before.log
```

生ログは `scratch/results/` に置かない（AGENTS.mdの方針通りraw dumpは非コミット）。
