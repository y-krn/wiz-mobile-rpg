...
core装備率の目標帯35〜40%との比較には条件差がある。#442の resolved env は `DEPARTURE_CRAFT_IDS=`（空）かつ `STATUS_CURE_HP_THRESHOLD=1` で、出発クラフトと状態異常治療を使わない。#442の37.2% [33.1%, 41.5%; N=500] はこの自己制限条件の値であり、現行基準線の実プレイ条件（出発クラフト7個・治療あり）の69.0% [68.2%, 69.8%; N=12000] と同じ目標の判定基準にはできない。目標帯の再設定はオーナー決定事項。

## A1

### 4職合算

`death = (deathFloor === 5)` なので、`reachedFloor >= 6` の層で B5死亡率が0%になるのは定義上の必然。したがってこの層別は、実質 `reachedFloor === 5` の層と B5到達者分母の構成を記述するだけで、選別効果の判定には使えない。
...
| Q | `reachedFloor===5` N | exact-5層のB5死亡率 | B5到達者 N | B5到達者のうちB5停止 | B5到達者のうちB5突破 |
| ---: | ---: | --- | ---: | --- | --- |
| Q1 | 502 | 45.8% [41.5%, 50.2%; N=502] | 702 | 32.8% [29.4%, 36.3%; N=702] | 28.5% [25.3%, 31.9%; N=702] |
| Q2 | 464 | 39.7% [35.3%, 44.2%; N=464] | 701 | 26.2% [23.1%, 29.6%; N=701] | 33.8% [30.4%, 37.4%; N=701] |
| Q3 | 420 | 35.5% [31.1%, 40.2%; N=420] | 702 | 21.2% [18.4%, 24.4%; N=702] | 40.2% [36.6%, 43.8%; N=702] |
| Q4 | 407 | 41.5% [36.8%, 46.4%; N=407] | 700 | 24.1% [21.1%, 27.4%; N=700] | 41.9% [38.3%, 45.5%; N=700] |

Q4はQ3よりB5突破割合が高い（41.9%対40.2%）。この分母構成差はQ3→Q4反転に寄与しうるが、選別効果の成立を示すものではない。選別効果の検証には、`deathFloor === floor` の定義上のトートロジーを含まない別の層別が必要。
...
- 内訳: endpoint率40（初回率4 + 各職B1/B5/B10の突破/死亡/撤退36。entrantは母数表示）、Q4/core 8、A1 4。合計52。
