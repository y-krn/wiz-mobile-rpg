...
- sim側の宝箱解除判断閾値=50.0%。TRAP_KITがあれば先に確定成功、無ければ chance >= 閾値だけ直接解除を試み、未満なら強行する実経路（scratch/sim_depth_material_ev.js:resolveChestTrapForSimulation）。
