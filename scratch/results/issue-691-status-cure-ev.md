...
- 観測値の出所: `scratch/results/issue-689-status-cure.md`
- 出力の時刻・CPU計測は `ISSUE689_DETERMINISTIC=1` で固定。シミュレーションの乱数・経路は変更しない。

4職の行集計は既存の `scratch/sim_commit_depth_624.js` を使用した。この harness は実際の
`scratch/sim_depth_material_ev.js` の `simulateRun` / `generateRunFloor` 経路を呼ぶ。
...
env SIM_SEED=231 SIM_RUNS=500 SIM_CALIBRATION_RUNS=100 \
  ...
  ISSUE624_CONDITION_ID=ev-691 node scratch/sim_commit_depth_624.js
```
...
- `node --check scratch/test_status_cure_ev.js`: PASS
