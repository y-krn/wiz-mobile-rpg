...
- 生出力: `scratch/results/issue-271-resistance-integrity-baseline.raw.txt`
...
- 生出力: `scratch/results/issue-271-resistance-integrity-src-after.raw.txt`
...
- `SIM_PARALLEL=1 node scratch/sim_depth_material_ev.js`: exit 0。
- `SIM_PARALLEL=15 node scratch/sim_depth_material_ev.js`: exit 0。
- 上記逐次/15並列stdout: `cmp` 完全一致。
- `node scratch/sim_workshop_progression.js`: exit 0。
- `node scratch/test_sim_reward_paths.js`: 23 sim fileすべて単一reward/level path、exit 0。
- `node scratch/sim_issue_271_resistance_integrity.js`: baseline・全what-if・実src各exit 0。
