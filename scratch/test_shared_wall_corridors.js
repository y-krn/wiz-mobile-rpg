...
function isPassageCell(cell) {
  return cell.walls.some(wall => !wall);
}

function isInsideRoom(rooms, x, y) {
...
function findSharedWallViolations(grid, rooms) {
  const violations = [];
  for (let y = 0; y < grid.length; y++) {
    for (let x = 0; x < grid[y].length; x++) {
      ...
      for (const { dir, dx, dy, opposite } of EDGE_DIRS) {
        ...
        if (!isPassageCell(cell) || !isPassageCell(next)) continue;
        if (isInsideRoom(rooms, x, y) || isInsideRoom(rooms, nx, ny)) continue;
...
for (let seedIndex = 0; seedIndex < SEED_COUNT; seedIndex++) {
  ...
  for (let floor = 1; floor <= FLOOR_COUNT; floor++) {
    ...
    const violations = findSharedWallViolations(generated.grid, generated.rooms);
...
assert.equal(violationCount, 0,
  `found ${violationCount} shared-wall corridor edges; first=${JSON.stringify(firstViolation)}`);
assert(averageDeadEnds >= DEAD_END_RANGE[0] && averageDeadEnds <= DEAD_END_RANGE[1],
  `average dead ends ${averageDeadEnds.toFixed(2)} outside ${DEAD_END_RANGE[0]}-${DEAD_END_RANGE[1]}`);
