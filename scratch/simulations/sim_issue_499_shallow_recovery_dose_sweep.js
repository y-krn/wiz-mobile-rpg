...
function buildMarkdown(summary) {
  ...
  lines.push(`- 床罠察知: ${summary.environment.ISSUE499_FIXED_DETECTION === "1" ? "確定（既存の罠ボーナスは解除へ転換）" : "source既定"}`);
...
async function main() {
  ...
  const reproductionCommand = `${fixedArgs} node scratch/simulations/sim_issue_499_shallow_recovery_dose_sweep.js`;
  const summary = {
    issue: 499,
  ...
  console.log(JSON.stringify({
    output: `evidence/results/${OUTPUT_STEM}.md`,
    summaryOutput: `evidence/results/${OUTPUT_STEM}.json`,
