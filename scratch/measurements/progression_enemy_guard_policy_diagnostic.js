...
function describe(result, { context, level, arm, seed, runIndex }) {
  ...
  return {
    ...
    maxHp,
    playerBeforeAnyEnemy: rounds[0]?.playerActionExecutionTiming === "player-before-any-enemy",
...
function makeSummary(report) {
  return [
    ...
    "",
    "| Context / Level / arm | N | Survival | Player before enemy | Rounds p50 | Enemy actions p50 | Damage dealt p50 | Damage taken p50 | Damage / enemy action p50 | Post HP p50 | Selected attack / defend | Executed attack / defend | Guard opportunities | Guarded enemy actions | maxHP | raw DEF |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ...report.summary.map(row => `| ${row.key} | ${row.n} | ${(row.survivalRate * 100).toFixed(1)}% | ${(row.playerBeforeAnyEnemyRate * 100).toFixed(1)}% | ${row.rounds.p50 ?? "—"} | ${row.enemyActions.p50 ?? "—"} | ${row.damageDealt.p50 ?? "—"} | ${row.damageTaken.p50 ?? "—"} | ${row.damagePerEnemyAction.p50 ?? "—"} | ${row.postCombatHp.p50 ?? "—"} | ${row.selectedActions.attack.mean ?? "—"} / ${row.selectedActions.defend.mean ?? "—"} | ${row.executedActions.attack.mean ?? "—"} / ${row.executedActions.defend.mean ?? "—"} | ${row.guardOpportunities.mean ?? "—"} | ${row.guardedEnemyActions.mean ?? "—"} | ${row.maxHp.mean ?? "—"} | ${row.rawDefense.mean ?? "—"} |`),
    "",
