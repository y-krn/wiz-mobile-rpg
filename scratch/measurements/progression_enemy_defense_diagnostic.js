...
import { deriveProgressionEnemyRunSeed, PROGRESSION_ENEMY_DEFENSE_ARMS as ARMS, PROGRESSION_ENEMY_DEFENSE_CANDIDATE as CANDIDATE, PROGRESSION_ENEMY_DIAGNOSTIC_CONTEXTS } from "./progression_enemy_defense_contract.js";
import { MONSTERS } from "../../src/data/monsters.js";
import { requireRunnerProvenance } from "./measurement_provenance.js";
import { printEnvSignatureBanner, readSimScopeDeclaration } from "./measurement_env_signature.js";

export const RUNNER_VERSION = "progression-enemy-defense-diagnostic-v3";
export const SCHEMA_VERSION = 1;
export const RUNNER_PATH = "scratch/measurements/progression_enemy_defense_diagnostic.js";
const DEFAULT_RUNS = 200;
const DEFAULT_SEED = 1694;
...
export async function runProgressionEnemyDefenseDiagnostic({ runs = DEFAULT_RUNS, seed = DEFAULT_SEED } = {}) {
    ...
    for (const fixture of FIXTURES) {
      for (const level of [1, 2]) for (let runIndex = 0; runIndex < count; runIndex++) {
        ...
        for (const arm of ARMS) {
          ...
          observations.push(describe(result, { context, fixture: fixtureAtLevel, arm: arm.id, seed: pairedSeed, runIndex }));
        }
      }
    }
  }
  ...
  return {
    ...
    configuration: { runs: count, seed: rootSeed, pairedSeedKey: ["root seed", "milestone context", "fixture", "runIndex"], pairedArmsShareSeed: true, contexts: PROGRESSION_ENEMY_DIAGNOSTIC_CONTEXTS, fixtures: FIXTURES, arms: ARMS.map(arm => ({ id: arm.id, rawDefenseBonus: arm.id === "v1" ? 0 : "baseline > 0 ? 0.5 : 0" })), candidate: CANDIDATE.id },
    observations,
    summary
  };
}
...
function makeSummary(report) {
  return [
    ...
    `- Phase 4c v1 vs def-plus-half-capped raw DEF baseline; PR N<30 is correctness evidence only`,
...
async function main() {
  ...
  const provenance = requireRunnerProvenance({ fetchOriginMain: false, measurementRunnerPaths: [RUNNER_PATH, "scratch/measurements/progression_enemy_defense_contract.js", "scratch/measurements/progression_enemy_candidate_contract.js", "scratch/measurements/progression_enemy_candidate_level.js", "scratch/simulations/sim_depth_material_ev.js", "src/data/progression_enemy_simulation_policy.js"] });
  const report = await runProgressionEnemyDefenseDiagnostic({ runs, seed: options.seed || DEFAULT_SEED });
  const measurement = {
    ...
    policyVersion: "phase4f-progression-enemy-defensive-baseline-half-capped-v1",
    candidate: { id: CANDIDATE.id, baseline0to5: [0, 5], enemyBand0to5: [0, 5], physical: "1 + 0.16 × baseline", spell: "1 + 0.16 × baseline", level1MaxHp: "20 × (1 + 0.10 × baseline)", arms: [{ id: "v1", rawDefenseBonus: 0 }, { id: "def-plus-half-capped", rawDefenseBonus: "baseline > 0 ? 0.5 : 0" }], enemyHp: "1 + 0.20 × band", enemyAttack: "1 + 0.10 × band", enemyDefense: 1 },
    workflow: {
