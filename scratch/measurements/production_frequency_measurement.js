...
function finalizeFrequencyDominance(dominance, weighting) {
  ...
  return { metric: "diagnostic utility", ...finalizeBestShare(dominance.votes, dominance.encounters, weighting, "fractional utility ties"), familyDepth };
}
...
function renderSensitivity(sensitivity) {
  ...
  return [`- strict significant reversal count: **${sensitivity.strictReversalSummary.strictSignificantReversalCount}**`, `- strict reversal rule: paired outcome + utility bootstrap 95% CIs, both signs reversed; minimum paired N **${sensitivity.strictReversalSummary.minimumPairedN}**`, `- insufficient-sample family comparisons excluded: **${sensitivity.strictReversalSummary.insufficientSampleComparisons}**`, `- family paired N: **${sensitivity.familyPairSampleSizes.length}** build-pair×family entries recorded in JSON`, `- equal-cell best-build coverage (not encounter-frequency weighted): **${equal.dominantBuild || "n/a"}**, share **${equal.dominantShare === null ? "n/a" : (equal.dominantShare * 100).toFixed(2) + "%"}** across **${equal.totalWeight}** depth×family cells`, `- production-frequency-weighted best-build share (diagnostic utility): **${weighted?.dominantBuild || "n/a"}**, share **${weighted?.dominantShare === null || weighted?.dominantShare === undefined ? "n/a" : (weighted.dominantShare * 100).toFixed(2) + "%"}** across **${weighted?.totalWeight || 0}** encounter samples`, "", "| Build pair | Paired clear difference (left − right) | Paired HP difference (left − right) | Paired MP difference (left − right) |", "| --- | ---: | ---: | ---: |", ...sensitivity.pairwiseOverall.map(row => `| ${row.leftBuildId} vs ${row.rightBuildId} | ${format(row.leftMinusRight?.clearRate)} | ${format(row.leftMinusRight?.hpPreservation)} | ${format(row.leftMinusRight?.mpPreservation)} |`)].join("\n");
}

function renderFamilyPairSamples(sensitivity) {
  const rows = sensitivity.familyPairSampleSizes.slice().sort((left, right) => `${left.family}:${left.leftBuildId}:${left.rightBuildId}`.localeCompare(`${right.family}:${right.leftBuildId}:${right.rightBuildId}`)).map(row => `| ${row.leftBuildId} vs ${row.rightBuildId} | ${row.family} | ${row.pairedN} |`);
  return ["### Family paired sample sizes", "", "Every family/build-pair paired N is recorded here. N<" + MIN_STRICT_PAIRED_N + " is emitted as `insufficient_sample` and cannot enter strict reversal.", "", "| Build pair | Family | Paired N |", "| --- | --- | ---: |", ...rows, ""].join("\n");
}
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    else if (value === "--help") { console.log("Usage: node scratch/measurements/production_frequency_measurement.js --runs 5000 --stress-runs 500 --seed 987-production-frequency --output evidence/results/issue-987-production-frequency.json --summary evidence/results/issue-987-production-frequency.md"); process.exit(0); }
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const provenance = requireRunnerProvenance({ fetchOriginMain: false, measurementRunnerPaths: ["scratch/measurements/production_frequency_measurement.js", "scratch/measurements/build_sensitivity_measurement.js", "src/combat_logic/round.js", "scratch/measurements/measurement_provenance.js", "scratch/measurements/measurement_env_signature.js"] });
