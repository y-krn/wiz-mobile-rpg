...
function getRoundMetric(sample) {
  ...
  return {
    ...
    normalDamageTotal: physicalEvents.reduce((sum, event) => sum + event.finalDamage, 0),
    normalAttacksReceivedPerRound: sample.trace.map(round => ({ round: round.round, value: physicalEvents.filter(event => event.round === round.round).length })),
...
function getPureRawDeathMetrics(sample, buildId, encounterId, depth) {
  ...
  return {
    ...
    normalHitDamages: roundMetric.physicalHitDamages,
    ordinaryNormalHitDamages: roundMetric.ordinaryNormalHitDamages,
    normalAttackCount: roundMetric.physicalHitCount,
...
function summarizeCells(cases) {
  ...
  return { runs, legacyRawDamageDeaths: raw, pureRawDamageDeaths: pure, pureRawDeathRate: pure / Math.max(1, runs), pureRawShareOfLegacyRaw: pure / Math.max(1, raw) };
}
...
function createPairedAggregate() {
  return { pairedRuns: 0, baselinePureRawDeaths: 0, counterfactualPureRawDeaths: 0, pureRawDeathsAvoided: 0, pureRawDeathsRemaining: 0, candidateDeathAfterBaselinePureRaw: 0, candidateClearAfterBaselinePureRaw: 0, causeShiftCounts: {} };
}
...
function addPairedSample(aggregate, baseline, candidate) {
  ...
  aggregate.pairedRuns++; aggregate.baselinePureRawDeaths += Number(baselinePure); aggregate.counterfactualPureRawDeaths += Number(candidatePure);
...
function renderSummary(report) {
  ...
  const counterRows = candidates.map(condition => `| ${condition.id} | ${condition.paired.baselinePureRawDeaths} | ${condition.paired.counterfactualPureRawDeaths} | ${(condition.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp | ${(condition.paired.baselinePureRawDeathsAvoidedRate * 100).toFixed(2)}% |`);
  ...
  return [
    ...
    "## Counterfactual paired comparison", "", "Reduction is the paired change in pure-raw incidence, not a tuning target. A candidate may shift a death to another category; those shifts remain visible in JSON.", "", "| Condition | Baseline pure raw | Counterfactual pure raw | Reduction | Baseline pure raw runs avoided |", "| --- | ---: | ---: | ---: | ---: |", ...counterRows, "", `Measured contribution order: ${candidates.map(condition => `${condition.id} ${(condition.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp`).join(" > ")}. This ranks fixed experiments; it does not recommend applying them in production.`, "",
    "## Required answers", "", `1. **主因:** ${candidates[0]?.id || "unmeasured"} has the largest isolated paired reduction; other effects are ${candidates.slice(1).map(condition => `${condition.id} ${(condition.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp`).join(", ")}. Multiple factors remain if their reductions are material; no 60% threshold is used.`, `2. **寄与順位:** ${candidates.map((condition, index) => `${index + 1}. ${condition.id}`).join("、")}。`, `3. **Build差:** build-specific pure-raw rates and metrics are reported below and in all 144 baseline cells; the structure is ${new Set(baseline.byBuild.map(row => row.pureRawDeathRate.toFixed(3))).size > 1 ? "not identical across builds" : "similar across builds"}.`, "4. **Depth差:** depth rows plus each cell's rounds, incoming attacks, total normal damage, enemy count, and HP-removal speed show what changes from B8 to B30; depth is not treated as an independent cause.", `5. **Fixture極端さ:** controlled fixtures average ${fixture.averageEnemyCount.toFixed(2)} enemies; production generation sampled at the same depths averages ${productionAverage.toFixed(2)}. The JSON records per-depth sizes and compositions.`, "6. **次の本番レバー:** **まだ触らない**。Validate production-frequency-weighted results first; this evidence does not authorize changing enemy HP/ATK, Mage, defense, pools, or action rules.", "7. **#973 Build Confidence:** **Revise** — build interaction is measurable, but a single global confidence conclusion would ignore encounter weighting and processing speed.", "",
