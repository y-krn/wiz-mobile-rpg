...
function renderSummary(report) {
  ...
  return [
    ...
    "## Required answers", "", `1. **Single-hit normal physical damage:** C4 shows the largest isolated pure-raw reduction (${(candidates.find(condition => condition.id === "C4_single_hit_damage")?.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp).`, `2. **Fight duration / processing time:** C3's enemy-HP reduction/shortening is also large (${(candidates.find(condition => condition.id === "C3_fight_duration")?.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp).`, `3. **Multi-enemy → single:** C1 is material (${(candidates.find(condition => condition.id === "C1_multi_enemy_to_single")?.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp), but it combines enemy count, composition, role/trait, targeting, and interactions; enemy-count-only contribution is unresolved.`, "4. **MultiAction extra action:** C2 did not improve pure raw; multiAction extra actions were not supported as the main cause in this controlled measurement.", "5. **Total enemy action count:** unresolved. C2 leaves one ordinary action per living enemy, so it does not independently test total enemy actions per round.", `6. **Controlled fixtures vs production generation:** controlled fixtures average ${fixture.averageEnemyCount.toFixed(2)} enemies; production generation sampled at the same depths averages ${productionAverage.toFixed(2)}. The latter is generated-distribution sampling, not observed full-run encounter frequency; controlled death rates are not global game death rates.`, "7. **Next production lever:** **まだ触らない**。Production-frequency-weighted measurement comes first; this evidence does not authorize changing enemy HP/ATK, Mage, defense, pools, encounter generation, or action rules.", "8. **#973 Build Confidence:** **Revise** — build interaction is measurable, but counterfactual scope and encounter weighting must be clarified before a production decision.", "",
