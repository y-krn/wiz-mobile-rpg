...
export const RUNNER_VERSION = "issue984-pure-raw-decomposition-v2";
...
export const COUNTERFACTUALS = Object.freeze([
  ...
  { id: "C1_multi_enemy_to_single", label: "C1: multi-enemy fixture simplified to one", kind: "multi_enemy_to_single" },
  { id: "C2_disable_multi_action_extra", label: "C2: disable multiAction extra action", kind: "disable_multi_action_extra" },
...
function getRoundMetric(sample) {
  ...
  return {
    ...
    enemyActionsPerRound: sample.trace.map(round => ({
      round: round.round,
      value: round.enemyTurnEvents ? round.enemyTurnEvents.length : round.enemyActions.length
    })),
...
export function runDecomposition({ seed = DEFAULT_SEED, runs = DEFAULT_RUNS, provenance = null } = {}) {
  ...
  return {
    ...
    interpretation: {
      classificationBaseline: "PR #983 exclusive pure_raw_damage category",
      primaryMetric: "pure raw death incidence per paired run; no 60% target",
      counterfactualInterpretation: {
        C1_multi_enemy_to_single: "multi-enemy fixture simplified to its first production monster; combined enemy-count, composition, role/trait, targeting, and multi-enemy-interaction effect; not an enemy-count-only estimate",
        C2_disable_multi_action_extra: "suppresses only the extra action queued by the multiAction trait; ordinary one-turn-per-living-enemy actions remain; total enemy-action-count contribution is unresolved",
        C3_fight_duration: "enemy HP fixed at 50% while enemy ATK and all other enemy stats remain unchanged; fight-duration / processing-time effect",
        C4_single_hit_damage: "normal physical incoming damage fixed at 50% after ordinary mitigation; status-payoff and snipe physical damage are unchanged; single-hit effect"
      },
      conclusionLimits: [
        "C1 is a multi-enemy-to-single composition simplification, not a pure enemy-count counterfactual",
        "C2 does not set total enemy actions per round to one; it only disables multiAction extra actions",
        "controlled fixture rates are not production encounter-frequency-weighted global death rates"
      ]
    }
  };
}
...
function renderSummary(report) {
  ...
  return [
    ...
    "## Scope and validity", "", "PR #983's exclusive `pure_raw_damage` category is the baseline. The baseline condition reuses the same causal runner and production round path; C1–C4 are measurement-only hooks with one changed factor each. Every condition uses the exact same derived case seed for paired comparison.", "", "Counterfactual meanings: **C1_multi_enemy_to_single** keeps only the first production monster from a multi-enemy fixture, so enemy count, composition, role/trait, target priority, and multi-enemy interactions change together; it is not an enemy-count-only estimate. **C2_disable_multi_action_extra** suppresses only `multiAction`'s extra action; ordinary multiple enemies still each retain their normal one action per round, so total enemy-action-count contribution remains unresolved. C3 halves enemy HP only; C4 halves ordinary normal-physical incoming damage only.", "", "Modeled: production monster definitions/depth scaling, auto-action, spell/status/mitigation rules, and the #983 causal classifier. Omitted: map traversal, encounter frequency in a complete run, manual input, consumables/retreat, and between-encounter progression. Controlled fixtures must not be interpreted as the game's global death rate.", "",
    ...
    "## Counterfactual paired comparison", "", "Reduction is the paired change in pure-raw incidence, not a tuning target. A candidate may shift a death to another category; those shifts remain visible in JSON. Total-death change is shown separately to expose label shifts.", "", "| Condition | Baseline pure raw | Counterfactual pure raw | Pure-raw reduction | Total death reduction | Baseline pure raw runs avoided |", "| --- | ---: | ---: | ---: | ---: | ---: |", ...counterRows, "", `Measured fixed-condition effects: ${candidates.map(condition => `${condition.id} ${(condition.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp`).join("; ")}. C4 is the largest isolated effect and C3 is also large; C1 is a mixed composition simplification; C2 tests only multiAction extra actions. This is not an additive contribution ranking and does not recommend production tuning.`, "",
    "## Required answers", "", `1. **主因:** C4_single_hit_damage shows the largest isolated pure-raw reduction (${(candidates.find(condition => condition.id === "C4_single_hit_damage")?.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp); C3's enemy-HP reduction/shortening is also large (${(candidates.find(condition => condition.id === "C3_fight_duration")?.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp). C1's multi-enemy-to-single simplification is material (${(candidates.find(condition => condition.id === "C1_multi_enemy_to_single")?.paired.pureRawDeathRateReduction * 100).toFixed(2)}pp), but its enemy-count-only contribution cannot be separated.`, "2. **寄与順位:** C4 is the largest isolated effect and C3 is also large; C1 is a mixed composition simplification; C2 tests only multiAction extra actions. These fixed experiments are not additive and do not establish a total-enemy-action ranking.", `3. **Build差:** build-specific pure-raw rates and metrics are reported below and in all 144 baseline cells; the structure is ${new Set(baseline.byBuild.map(row => row.pureRawDeathRate.toFixed(3))).size > 1 ? "not identical across builds" : "similar across builds"}.`, "4. **Depth差:** depth rows plus each cell's rounds, incoming attacks, total normal damage, enemy count, and HP-removal speed show what changes from B8 to B30; stronger normal hits are the clearest depth signal, with processing-time interaction secondary.", `5. **Fixture極端さ:** controlled fixtures average ${fixture.averageEnemyCount.toFixed(2)} enemies; production generation sampled at the same depths averages ${productionAverage.toFixed(2)}. The JSON records per-depth sizes and compositions. This is generated-distribution sampling, not observed full-run encounter frequency, and controlled death rates are not global game death rates.`, "6. **次の本番レバー:** **まだ触らない**。Production-frequency-weighted measurement comes first; this evidence does not authorize changing enemy HP/ATK, Mage, defense, pools, encounter generation, or action rules.", "7. **#973 Build Confidence:** **Revise** — build interaction is measurable, but counterfactual scope and encounter weighting must be clarified before a production decision.", "",
