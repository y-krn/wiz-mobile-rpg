...
function renderSummary(report) {
  const lines = [
    ...
    "",
    "| Band | Main non-empty | Equipment exposure/decision | Rune exposure/decision | Consumable exposure/decision | Target pivot | Actual pivot | True pivot adopted | Core find 0/1/2+ | Core equip 0/1/2+ |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- |",
    ...report.bands.map(band => `| ${band.label} | ${formatRate(band.rates.mainNonEmpty)} | ${formatRate(band.rates.equipmentExposurePerDecision)} | ${formatRate(band.rates.runeExposurePerDecision)} | ${formatRate(band.rates.consumableExposurePerDecision)} | ${formatRate(band.rates.targetPivotExposure)} | ${formatRate(band.rates.actualPivotExposure)} | ${formatRate(band.rates.truePivotAdoptionPerPivotCandidate)} | \`${JSON.stringify(band.perRunDistributions.coreFinds)}\` | \`${JSON.stringify(band.perRunDistributions.coreEquipped)}\` |`),
    "",
    "## Rune supply",
    "",
