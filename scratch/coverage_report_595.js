...
function buildReport({
  ...
  const lines = [
    ...
    "## 測定条件",
    "",
    `- 正典sim: \`scratch/sim_depth_material_ev.js\``,
    ...
    "- 再現コマンド: `NODE_V8_COVERAGE=<coverage-dir> node scratch/sim_depth_material_ev.js`",
    "",
    "## ディレクトリ別関数カバー率",
    "",
    "| 対象 | 実行済み / 全関数 | カバー率 |",
...
if (!options.coverageDir) {
  throw new Error("usage: node scratch/coverage_report_595.js <coverage-dir> [sim-log] [output.md]");
}
